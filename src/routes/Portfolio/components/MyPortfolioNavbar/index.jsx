...
const MyPortfolioNavbar = () => {
  ...
  const isUserInterestedInNav = useRef(false);

  useEffect(() => {
    setTimeout(() => {
      if (!isUserInterestedInNav.current) setShowNav(false);
    }, 3000);
  }, []);

  const handleUserInterestOnNav = () => {
    isUserInterestedInNav.current = true;
  };

  return (
    <nav
      onClick={handleUserInterestOnNav}
      onMouseEnter={handleUserInterestOnNav}
      onTouchStart={handleUserInterestOnNav}
      className="bg-outer-space-crayola text-white fixed left-4 top-4 my-auto rounded-lg flex flex-col gap-2"
    >
